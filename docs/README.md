...
`plugins/ideo-scrum/`（交付物）之外的第二层：本仓库的开发史。每期 Sprint 留下一份**冲刺日志**，产品侧留下一份**产品日志**。
...
- **1.x 收官**（2026-08-30 前）：Sprint 01-05 完成，PBI-1~5 ✅；PBI-6 已并入 v2.0.0 的 PBI-7。
- **v2.0.0 新轨**（2026-08-30 起）：旧 Product Goal（结构对等）判定为方向性错误，详见 `ProductBacklog.md` 修订记录。PBI-7（ideo 与 Design Sprint 融合重构，前置 Design Sprint 验证轮）、PBI-8（角色体系）、PBI-9（双内核联动）、PBI-10（designer 重写）、PBI-11（方法论感悟背景文档）——待开始（草案）。
