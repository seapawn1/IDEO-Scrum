...
| PBI-19 | 验收双机制——DoD 自检与 SM 证据初验 | 完工必须自证：developer 完工交证据包（测试输出 + DoD 逐条 + diff 摘要），每次必做（DoD 自检层）；SM 于 Sprint Review 凭证据包核验、不凭记忆断言，出「验讫/存疑」结论（独立初验层）；高风险或增量重时升级为干净上下文 command/agent（分化预留，不预建） | 源为 2026-09-23 对话定案（seapawn.md 未记条目） | 待开始 | 定案（2026-09-23）：监理挂 SM 名下，同会话共享上下文之锚定风险 PO 判定可接受（证据化验收为兜底）；「挂 developer」议后定性为 DoD 自检层单列；初验清单与 PBI-17 的 review 产出规格衔接 |
